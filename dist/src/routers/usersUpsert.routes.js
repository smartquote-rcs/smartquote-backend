...
const upsertRouter = (0, express_1.Router)();
...
upsertRouter.post('/upsert', async (req, res) => {
    try {
        ...
        const { data: existing, error: selError } = await connect_1.default
            .from('users')
            .select('*')
            .ilike('email', email)
            .maybeSingle();
        if (selError)
            console.warn('Upsert user select error:', selError.message);
        if (existing) {
            ...
            if (needsUpdate) {
                await connect_1.default.from('users').update(updates).eq('id', existing.id);
            }
        ...
        const { data: inserted, error: insError } = await connect_1.default
            .from('users')
            .insert([newUser])
...
//# sourceMappingURL=usersUpsert.routes.js.map