...
export class ItemListRenderer {
  ...
  public async render(data: RelatorioData) {
    ...
    try {
      const itensData = await cotacoesItensService.list(data.cotacaoId);
      const itensEscolhidos = itensData.filter((item: any) => item.status === true);
      const itensNaoEscolhidos = itensData.filter((item: any) => item.status === false);
...
private renderizarItemAnalise(item: any, index: number, analise: any, isLocal: boolean, contentWidth: number) {
  ...
  if (analise.escolha_principal && analise.escolha_principal !== descricao) {
    ...
    const analiseHeight = this.doc.heightOfString(
      ...
      { width: contentWidth - 20, lineGap: 1 }
    );
  ...
  if (analise.escolha_principal && analise.escolha_principal !== descricao) {
    this.doc
      ...
      .font('Times-Italic')
      .text(
        `Análise: ${analise.escolha_principal}`,
        margin + 18,
        currentY,
        { width: contentWidth - 20, lineGap: 1 }
      );
  ...
  private renderizarCardNaoEncontrado(index: number, analise: any, isLocal: boolean, contentWidth: number) {
    ...
    this.doc
      ...
      .fillAndStroke('#fff3cd', theme.warning.stroke)
      .fill(theme.warning.main)
      .fontSize(10)
      .font('Helvetica-Bold')
    ...
    if (analise?.escolha_principal) {
      this.doc
        ...
        .fillAndStroke('#fef3e2', theme.warning.stroke);
      this.doc
        .fill(theme.warning.main)
        .fontSize(8)
        .font('Helvetica-Bold')
   ...
   */
  private renderizarItemCotacao(item: any, index: number, contentWidth: number, isEscolhido: boolean) {
    const margin = this.margin;
   ...
   */
  private renderizarCardNaoEscolhido(index: number, item: any, contentWidth: number) {
    const margin = this.margin;
    const minHeight = 130;
    this.verificarEspacoPagina(minHeight);
    const itemY = this.doc.y;
   ...
   */
  private renderFallback(data: RelatorioData, contentWidth: number) {
    const margin = this.margin;
