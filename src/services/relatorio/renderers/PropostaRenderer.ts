...
public async render(data: RelatorioData) {
  ...
  const countSelectedItems = (items: any[], isLocal: boolean): number =>
    items.reduce((count, item) => {
      ...
      if (!choice || ranking.length === 0) return count;
      const found = ranking.some((r: any) =>
        normalizeText(r?.nome?.toString() || '').includes(choice) ||
  ...
  try {
    const quote = await CotacoesService.getById(data.cotacaoId);
  ...
  this.doc
    ...
    .fillColor('#003087') // Primary blue
    .text(
      budget.toLocaleString('pt-AO', { style: 'currency', currency: 'AOA', minimumFractionDigits: 2 }),
      margin + 15 + colWidth,
      infoY + 10
    );
