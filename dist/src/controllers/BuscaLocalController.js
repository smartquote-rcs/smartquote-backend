...
const PythonApiClient_1 = require("../services/PythonApiClient");
...
class BuscaLocalController {
    async searchLocal(req, res) {
        try {
            ...
            const result = await PythonInterpretationProcessor_1.pythonProcessor.processHybridSearch(searchPayload);
            if (!result.success) {
    ...
    async search(req, res) {
        try {
            ...
            if (searchWeb && faltantes.length === 0 && cotacoesInfo?.principal_id) {
                try {
                    const placeholdersDB = await CotacoesItensService_1.default.listPlaceholders(Number(cotacoesInfo.principal_id));
                    ...
                    console.warn('⚠️ [BUSCA-LOCAL] Falha ao carregar placeholders do banco:', e?.message || e);
                }
            }
            if (faltantes.length > 0 && searchWeb) {
                ...
                const { resultadosCompletos: resultados, produtosWeb: aprovados } = await svc.createJobsForFaltantesWithReforco(faltantes, solicitacao, ponderacaoWeb_LLM);
            ...
            if (cotacaoPrincipalId) {
                try {
                    const pendentes = await CotacoesItensService_1.default.listPlaceholders(Number(cotacaoPrincipalId));
            ...
            if (cotacaoPrincipalId && resultadosCompletos.length > 0 && searchWeb) {
                ...
                try {
                    ...
                    try {
                        const placeholdersRestantes = await CotacoesItensService_1.default.listPlaceholders(Number(cotacaoPrincipalId));
                        const novoStatus = placeholdersRestantes.length === 0 ? 'completa' : 'incompleta';
                        await CotacoesService_1.default.updatePartial(Number(cotacaoPrincipalId), { status: novoStatus });
                    }
                    catch { }
     ...
     */
    async checkPythonApiHealth(req, res) {
        try {
            ...
            if (isAvailable) {
                ...
                return res.status(200).json({
                    ...
                    health,
                    stats: PythonApiClient_1.pythonApiClient.getStats()
                });
            }
            else {
    ...
    async syncProducts(req, res) {
        try {
            ...
            const result = await PythonInterpretationProcessor_1.pythonProcessor.syncProducts();
            if (result.success) {
