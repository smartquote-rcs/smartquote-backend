...
function converterPrecoParaCentavos(precoString) {
    try {
...
casosTeste.forEach((teste, index) => {
    ...
    if (floatOk && centavosOk) {
        ...
        console.log(`   ❌ FALHOU`);
        if (!floatOk)
            console.log(`      - parseNumero incorreto: ${resultadoFloat} ≠ ${teste.expected}`);
        if (!centavosOk)
            console.log(`      - centavos incorreto: ${resultadoCentavos} ≠ ${teste.expectedCentavos}`);
        falharam++;
    }
});
console.log('\n' + '═'.repeat(80));
...
//# sourceMappingURL=test-price-parsing.js.map