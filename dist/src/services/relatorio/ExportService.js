...
class ExportService {
     ...
     */
    async buscarDadosResumo(cotacaoId) {
    ...
    id, status, orcamento_geral, prompt_id,
      ...
      `)
        ...
        const { data: itens, error: itensError } = await connect_1.default
            .from('cotacoes_itens')
            .select('*')
        ...
        const faltantesCompat = (itens || [])
            ...
            .map((i) => i.pedido || i.item_nome || 'Item solicitado');
        return {
     ...
     */
    aplicarEstilizacao(sheet, numRows, numCols) {
     ...
     */
    aplicarEstilizacaoTabela(sheet, headerRow, numDataRows, numCols) {
