...
public adicionarSecaoAnaliseWeb(doc: PDFKit.PDFDocument, data: RelatorioData) {
  ...
  analiseWeb.forEach((relatorioWeb, index) => {
    ...
    const statusAnalise = (relatorioWeb as any).status;
    ...
    const criteriosFonte = (relatorioWeb as any).criterios_avaliacao || (relatorioWeb as any).criterios_aplicados;
    ...
    // ========== Observations ==========
    const observacoes = (relatorioWeb as any).observacoes;
    if (observacoes) {
      this.verificarEspacoPagina(doc, 50);
      const obsY = doc.y;
