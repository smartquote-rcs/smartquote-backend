...
exports.AnaliseExternaRenderer = void 0;
class AnaliseExternaRenderer {
    verificarEspacoPagina(doc, altura) {
        if (doc.y + altura > doc.page.height - doc.page.margins.bottom) {
            doc.addPage();
        }
    }
    adicionarSecaoAnaliseExterna(doc, data) {
        ...
        // Data handling
        const analiseExterna = Array.isArray(data.analiseExterna)
            ? data.analiseExterna
            : data.analiseExterna
                ? [data.analiseExterna]
                : [];
        ...
        analiseExterna.forEach((relatorio, index) => {
            if (index > 0)
                doc.addPage();
            const webY = doc.y;
            // Metadata row (no explicit per-item header text, consistent with updated web renderer)
            const timestamp = relatorio.timestamp || Date.now();
            const produtosAnalisados = Array.isArray(relatorio.top_ranking) ? relatorio.top_ranking.length : 0;
            const statusAnalise = relatorio.status;
            const produtosSelecionados = statusAnalise === 'produto_adicionado' || relatorio.escolha_principal ? 1 : 0;
            doc
                .font('Helvetica')
                .fontSize(8)
                .fillColor('#6B7280')
                .text(`Data: ${new Date(timestamp).toLocaleDateString('pt-BR')} | Analisados: ${produtosAnalisados} | Selecionados: ${produtosSelecionados}`, margin + 10, webY + 5);
            // Status badge
            if (statusAnalise) {
            ...
            doc
                ...
                .stroke();
            doc.y += 10;
            const criteriosFonte = relatorio.criterios_avaliacao || relatorio.criterios_aplicados;
            const decisaoResumo = relatorio.escolha_principal ? String(relatorio.escolha_principal) : 'Não encontrada';
            ...
            if (relatorio.top_ranking && Array.isArray(relatorio.top_ranking) && relatorio.top_ranking.length > 0) {
                ...
                doc
                    ...
                    .stroke();
                doc.y += 15;
                const escolhaTexto = (relatorio.escolha_principal || '').toString().toLowerCase();
                relatorio.top_ranking.forEach((ranking, rankIndex) => {
                    let itemHeight = 30;
            ...
            // Observações
            const observacoes = relatorio.observacoes;
            if (observacoes) {
                this.verificarEspacoPagina(doc, 50);
                const obsY = doc.y;
...
exports.AnaliseExternaRenderer = AnaliseExternaRenderer;
//# sourceMappingURL=AnaliseExternaRenderer.js.map