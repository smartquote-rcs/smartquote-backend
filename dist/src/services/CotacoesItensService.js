...
class CotacoesItensService {
     ...
     */
    async insertPlaceholderItem(cotacaoId, faltante) {
        const payload = {
     ...
     */
    async fulfillPlaceholderWithAnalysis(cotacaoId, placeholderId, produto, quantidade, produtoId, relatorio, tipoBusca = 'principal') {
        const campoAnalise = tipoBusca === 'reforco' ? 'analise_web_externa' : 'analise_web';
        // Buscar análises existentes no campo adequado
        const { data: existing, error: fetchError } = await connect_1.default
            .from('cotacoes_itens')
            .select(campoAnalise)
            .eq('id', placeholderId)
            .single();
        ...
        const existentes = Array.isArray(existing?.[campoAnalise])
            ? existing[campoAnalise]
            : (existing?.[campoAnalise] ? [existing[campoAnalise]] : []);
     ...
     */
    async listPlaceholders(cotacaoId) {
        const { data, error } = await connect_1.default
            .from('cotacoes_itens')
            .select('*')
     ...
     */
    async removePlaceholderByIndex(cotacaoId, index) {
     ...
     */
    async removePlaceholderByPedido(cotacaoId, query) {
        // Buscar um placeholder cujo pedido contenha a query
        const { data: candidatos, error } = await connect_1.default
            .from('cotacoes_itens')
            .select('*')
            .eq('cotacao_id', cotacaoId)
            .eq('status', false)
        ...
        if (!alvo)
            return null;
        const { data, error: delErr } = await connect_1.default
            .from('cotacoes_itens')
            .delete()
            .eq('id', alvo.id)
            .select('*')
     ...
     */
    async removePlaceholderByNome(cotacaoId, nome) {
        const { data: candidatos, error } = await connect_1.default
            .from('cotacoes_itens')
            .select('*')
            .eq('cotacao_id', cotacaoId)
     ...
     */
    async insertWebItemById(cotacaoId, produtoId, produto, quantidade, relatorio) {
        const item_preco = this.parseNumero(produto.price);
        const provider = this.providerFromUrl(produto.product_url);
        const payload = {
            cotacao_id: cotacaoId,
     ...
     */
    async insertLocalItemById(cotacaoId, produtoId, produto, quantidade, analiseLocal) {
     ...
     */
    async updateItemWithAnaliseLocal(itemId, analiseLocal) {
        const { data, error } = await connect_1.default
            .from('cotacoes_itens')
     ...
     */
    async fulfillPlaceholderWithWebProduct(cotacaoId, placeholderId, produto, quantidade, produtoId, relatorio) {
        // Primeiro, buscar o placeholder existente para verificar se já tem relatório
        const { data: existingPlaceholder, error: fetchError } = await connect_1.default
            .from('cotacoes_itens')
            .select('analise_web')
            .eq('id', placeholderId)
            .single();
        ...
        const existingAnalise = Array.isArray(existingPlaceholder?.analise_web)
            ? existingPlaceholder.analise_web
            : (existingPlaceholder?.analise_web ? [existingPlaceholder.analise_web] : []);
        const analiseAtualizada = typeof relatorio !== 'undefined' && relatorio !== null
    ...
    async getSugeridosWeb(id) {
        try {
            ...
            if (relatorio && relatorio.analiseWeb && Array.isArray(relatorio.analiseWeb)) {
                const allSugs = relatorio.analiseWeb.flatMap((item) => item.top_ranking || []);
                const sugestoes = allSugs.map((item) => ({
                    nome: item.nome,
                    url: item.url,
                    preco: item.preco,
    ...
    async getSugeridosLocal(id) {
        try {
            ...
            if (relatorio) {
                const locaisArr = Array.isArray(relatorio.analiseLocal)
                    ? relatorio.analiseLocal
                    : [];
                const cacheArr = Array.isArray(relatorio.analiseCache)
                    ? relatorio.analiseCache
                    : [];
                const localSugs = locaisArr.flatMap((item) => item?.llm_relatorio?.top_ranking || []);
                const cacheSugs = cacheArr.flatMap((item) => (item?.llm_relatorio?.top_ranking || item?.top_ranking || []));
