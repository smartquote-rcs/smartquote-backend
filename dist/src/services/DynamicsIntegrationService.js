...
class DynamicsIntegrationService {
  ...
  hasAzureConfig: ${!!(this.config.azureClientSecret)}
     ...
     */
    async criarOpportunity(cotacao) {
        try {
     ...
     */
    async buscarOpportunityPorNome(name) {
        try {
            const token = await this.getOAuthToken();
     ...
     */
    async atualizarOpportunity(opportunityId, dados) {
        try {
            const token = await this.getOAuthToken();
     ...
     */
    async buscarQuotePorNumero(quotenumber) {
        try {
            const token = await this.getOAuthToken();
     ...
     */
    async atualizarQuote(quoteId, dados) {
        try {
            const token = await this.getOAuthToken();
     ...
     */
    async enviarQuoteDetails(quoteId, itens) {
     ...
     */
    async buscarPrecoDoProduto(produtoId) {
        try {
