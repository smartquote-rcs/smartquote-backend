...
class AnaliseCacheRenderer {
    ...
    adicionarSecaoAnaliseCache(doc, data) {
        ...
        doc
            ...
            .stroke();
        doc.y += 20;
        const analiseCacheArr = Array.isArray(data.analiseCache)
            ? data.analiseCache
            : data.analiseCache
                ? [data.analiseCache]
                : [];
        ...
        analiseCacheArr.forEach((analiseItem, index) => {
            const analise = analiseItem.llm_relatorio || analiseItem;
            if (index > 0) {
                doc.addPage();
            }
            const analiseY = doc.y;
            const pedido = analiseItem?.pedido;
            ...
            if (!hasQuery) {
                const detalhes = [
                    ...
                    `Item da cotação: ${String(analiseItem.item_nome || 'Sem nome')}`,
                    `ID do item: ${String(analiseItem.id_item_cotacao || '-')}`,
                ];
                const alternativas = Array.isArray(analise?.top_ranking) ? analise.top_ranking.length : 0;
                if (alternativas > 0) {
            ...
            doc.y += 10;
            const infoY = doc.y;
            const timestamp = analiseItem.timestamp || analise.timestamp || Date.now();
            ...
            if (analise.top_ranking && Array.isArray(analise.top_ranking)) {
                ...
                doc.y += 15;
                analise.top_ranking.forEach((ranking, rankIndex) => {
                    let itemHeight = 40;
                    if (ranking.nome) {
            ...
            if (analise.criterios_avaliacao) {
                ...
                criteriosList.forEach((criterio, index) => {
                    if (criterios[criterio.key]) {
                        ...
                        const criterioText = String(criterios[criterio.key]);
                        doc.font('Helvetica').fontSize(9);
...
exports.AnaliseCacheRenderer = AnaliseCacheRenderer;
//# sourceMappingURL=AnaliseCacheRenderer.js.map